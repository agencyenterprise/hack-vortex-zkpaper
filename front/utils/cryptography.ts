    ...
    switch (version) {
        case 'x25519-xsalsa20-poly1305': {
            ...
            const ephemeralKeyPair = {
                publicKey: Buffer.from(process.env.EPH_PUBLIC_KEY as string, 'base64'),
                secretKey: Buffer.from(process.env.EPH_SECRET_KEY as string, 'base64')
            }
            // console.log(Buffer.from(ephemeralKeyPair.secretKey).toString('base64'))
            // console.log(Buffer.from(ephemeralKeyPair.secretKey).toString('base64'))
            ...
            const nonce = Buffer.from(process.env.API_NONCE as string, 'base64')
